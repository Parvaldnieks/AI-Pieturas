    ...
    x-data="{ 
        ...
        get filtered() {
            ...
            return this.requests.filter(r =>
                (r.device_name ?? '').toLowerCase().includes(this.search.toLowerCase()) ||
                (r.requester_email ?? '').toLowerCase().includes(this.search.toLowerCase()) ||
        ...
        <table class="min-w-full dark:text-white text-center border-collapse">
            <thead>
                <tr class="border-b border-orange-500">
                    <th>{{ t('api.device', 'Ierīce') }}</th>
                    <th>{{ t('api.type', 'Tips') }}</th>
                ...
                <template x-if="filtered.length === 0">
                    <tr>
                        <td colspan="6" class="text-gray-500 italic py-4">
                            {{ t('api.no_results', 'Nekas netika atrasts.') }}
                        </td>
                    </tr>
                </template>
                        ...
                        <tr class="border-b border-orange-500">
                            ...
                            <td>
                                @if($request->requester_email)
                                    <span class="bg-blue-200 text-blue-800 px-2 py-1 rounded">{{ t('api.human', 'Lietotājs') }}</span>
                                @else
                                    <span class="bg-purple-200 text-purple-800 px-2 py-1 rounded">{{ t('api.machine', 'Ierīce') }}</span>
                                @endif
                            </td>
